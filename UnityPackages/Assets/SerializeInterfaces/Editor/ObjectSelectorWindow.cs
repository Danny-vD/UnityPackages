...
namespace AYellowpaper.Editor
{
	internal class ObjectSelectorWindow : EditorWindow
	{
		...
		private void BindItem(VisualElement listItem, int index)
		{
			...
			if (label != null)
			{
				GetText(_filteredItems[index], out string path, out string indexText, out string typeText);
				label.text = _filteredItems[index].Label + $"\t{typeText}\t\t[{path}]";
			}
		...
		private IEnumerable<ItemInfo> FetchAllComponents()
		{
			...
			{
				gameobjects = FindObjectsByType<GameObject>(FindObjectsInactive.Include);
			}
			else
			{
				gameobjects = currentPrefabStage.prefabContentsRoot.GetComponentsInChildren<Transform>().ToGameObject().ToArray();
			}
			...
			foreach (GameObject obj in gameobjects)
			{
				foreach (Component comp in obj.GetComponents(typeof(Component)))
				{
					if (CheckFilter(comp))
					{
						yield return new ItemInfo { Icon = EditorGUIUtility.ObjectContent(comp, comp.GetType()).image, InstanceID = comp.GetInstanceID(), Label = comp.name };
					}
				}
			}
		}
