...
namespace LocalisationPackage.Core.IO.Parsers
{
	...
	{
		private const string noLocalisationString = "NO-LOCALISATION";

		public static ILocalisationParser ParserImplementation;
		...
		/// If this also fails then the <see cref="noLocalisationString"/> will be returned<br/>
		...
		{
			if (ParserImplementation.CanPreReadAllEntries) // Because the parser can pre-read the entries, we can used the cached values
			{
